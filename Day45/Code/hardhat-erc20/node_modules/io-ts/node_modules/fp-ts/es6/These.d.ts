...
declare const bimap: <L, A, M, B>(f: (l: L) => M, g: (a: A) => B) => (fa: These<L, A>) => These<M, B>, foldMap: <M>(M: import("./Monoid").Monoid<M>) => <A>(f: (a: A) => M) => <L>(fa: These<L, A>) => M, map: <A, B>(f: (a: A) => B) => <L>(fa: These<L, A>) => These<L, B>, mapLeft: <L, M>(f: (l: L) => M) => <A>(fa: These<L, A>) => These<M, A>, reduce: <A, B>(b: B, f: (b: B, a: A) => B) => <L>(fa: These<L, A>) => B, reduceRight: <A, B>(b: B, f: (a: A, b: B) => B) => <L>(fa: These<L, A>) => B;
