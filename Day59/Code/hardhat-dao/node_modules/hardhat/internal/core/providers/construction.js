...
function createProvider(networkName, networkConfig, paths, artifacts, experimentalHardhatNetworkMessageTraceHooks = []) {
    ...
    if (networkName === constants_1.HARDHAT_NETWORK_NAME) {
        ...
        hardhatNetConfig.mining.mempool.order, hardhatNetConfig.chains, new ModulesLogger(hardhatNetConfig.loggingEnabled), accounts, artifacts, hardhatNetConfig.allowUnlimitedContractSize, hardhatNetConfig.initialDate !== undefined
            ? (0, date_1.parseDateString)(hardhatNetConfig.initialDate)
            : undefined, experimentalHardhatNetworkMessageTraceHooks, forkConfig, paths !== undefined ? getForkCacheDirPath(paths) : undefined, hardhatNetConfig.coinbase);
    }
    else {
